...
def patch_cfg_paths(cfg: Config,
    ...
    """
    # Resolve dataroot relative to workspace root if needed
    ...
    def _patch(node: Any) -> None:
        if isinstance(node, dict):
            ...
            if 'ann_file' in node:
                if ann_file:
                    # If ann_file is provided, resolve it relative to dataroot or workspace root
                    if osp.isabs(ann_file):
                        node['ann_file'] = ann_file
                    elif osp.exists(ann_file):
                        # Exists relative to current directory
                        node['ann_file'] = osp.abspath(ann_file)
                    else:
                        # Try relative to dataroot first
                        ann_path = osp.join(dataroot, ann_file)
                        if osp.exists(ann_path):
                            node['ann_file'] = osp.abspath(ann_path)
                        else:
                            # Try resolving relative to workspace root (handles cases like 'data/kitti/kitti_infos_val.pkl')
                            resolved_ann = resolve_path(ann_file)
                            if osp.exists(resolved_ann):
                                node['ann_file'] = resolved_ann
                            else:
                                # Last resort: use the joined path even if it doesn't exist yet
                                node['ann_file'] = osp.abspath(ann_path)
                else:
                    val = node['ann_file']
                    if osp.isabs(val):
                        ...
                        # Join with dataroot and resolve
                        ann_path = osp.join(dataroot, val)
                        node['ann_file'] = resolve_path(ann_path) if not osp.exists(ann_path) else osp.abspath(ann_path)
...
def build_loader_pack(
    ...
    if data_source == "cfg":
        ...
        if dataset.lower() == "nuscenes":
            ...
            #      Load directly from pkl file to ensure we get tokens.
            # ------------------------------------------------------------------
            sample_tokens = []
            try:
                # Load directly from pkl file (most reliable)
                import pickle
                ann_file_path = ann_file if ann_file else cfg.test_dataloader.dataset.get('ann_file', '')
                if ann_file_path and osp.exists(ann_file_path):
                    with open(ann_file_path, 'rb') as f:
                        pkl_data = pickle.load(f)
                        if 'data_list' in pkl_data:
                            for idx, info in enumerate(pkl_data['data_list']):
                                tok = info.get("token", None)
                                if tok is None:
                                    tok = str(idx)
                                sample_tokens.append(tok)
                        elif isinstance(pkl_data, list):
                            # Handle case where pkl is just a list
                            for idx, info in enumerate(pkl_data):
                                tok = info.get("token", None) if isinstance(info, dict) else None
                                if tok is None:
                                    tok = str(idx)
                                sample_tokens.append(tok)
                # Fallback: try dataset object if pkl loading failed
                if len(sample_tokens) == 0:
                    data_list = getattr(dataset_obj, "data_list", None) or getattr(dataset_obj, "data_infos", None)
                ...
                warnings.warn(f"Could not load sample_tokens: {e}")
                sample_tokens = []
...
def resolve_path(path: str) -> str:
    """
    Resolve a path relative to the workspace root if it doesn't exist as-is.
    
    This allows running scripts from subdirectories while using paths relative
    to the workspace root (e.g., 'mmdetection3d/configs/...').
    
    Args:
        path: Path to resolve (can be absolute, relative to CWD, or relative to workspace root)
        
    Returns:
        Resolved absolute path
    """
    # If path is absolute and exists, return as-is
    if osp.isabs(path) and osp.exists(path):
        return path
    
    # If path exists relative to current directory, return absolute version
    if osp.exists(path):
        return osp.abspath(path)
    
    # Try to find workspace root by looking for 'mmdetection3d' directory
    # Walk up from current directory
    current = osp.abspath(os.getcwd())
    while current != osp.dirname(current):  # Stop at filesystem root
        mmdet3d_dir = osp.join(current, 'mmdetection3d')
        if osp.exists(mmdet3d_dir) and osp.isdir(mmdet3d_dir):
            # Found workspace root
            resolved = osp.join(current, path)
            if osp.exists(resolved):
                return osp.abspath(resolved)
            break
        current = osp.dirname(current)
    
    # If still not found, return the original path (will raise error in caller)
    return osp.abspath(path) if not osp.isabs(path) else path
...
def run_benchmark_evaluation(args, sys_info: Dict[str, Any]) -> None:
    ...
    # ------------------------------------------------------------------
    # Resolve dataroot relative to workspace root
    resolved_dataroot = resolve_path(args.dataroot)
    
    # Resolve ann_file if provided
    resolved_ann_file = None
    if args.ann_file:
        # Explicit override from CLI - resolve relative to dataroot or workspace root
        if osp.isabs(args.ann_file):
            resolved_ann_file = args.ann_file
        elif osp.exists(args.ann_file):
            resolved_ann_file = osp.abspath(args.ann_file)
        else:
            # Try relative to dataroot first
            ann_path = osp.join(resolved_dataroot, args.ann_file)
            if osp.exists(ann_path):
                resolved_ann_file = osp.abspath(ann_path)
            else:
                # Try resolving relative to workspace root
                resolved_ann_file = resolve_path(args.ann_file)
    else:
        ...
        if default_ann:
            # Resolve default ann_file relative to dataroot
            ann_path = osp.join(resolved_dataroot, default_ann)
            resolved_ann_file = osp.abspath(ann_path) if osp.exists(ann_path) else resolve_path(default_ann)
    ...
    if hasattr(cfg, "test_dataloader") and hasattr(cfg.test_dataloader, "dataset"):
        ...
        if hasattr(ds, 'data_root'):
            print(f"[Runner Eval] Overriding dataset.data_root -> {resolved_dataroot}")
            ds.data_root = resolved_dataroot

        # ann_file override
        if resolved_ann_file:
            print(f"[Runner Eval] Overriding dataset.ann_file -> {resolved_ann_file}")
            ds.ann_file = resolved_ann_file
    ...
    def _patch_metric(metric_cfg: Dict[str, Any]) -> None:
        ...
        if 'data_root' in metric_cfg:
            metric_cfg['data_root'] = resolved_dataroot

        # ann_file override
        if resolved_ann_file:
            metric_cfg['ann_file'] = resolved_ann_file
        else:
            if 'ann_file' in metric_cfg and not osp.isabs(metric_cfg['ann_file']):
                ann_path = osp.join(resolved_dataroot, metric_cfg['ann_file'])
                metric_cfg['ann_file'] = osp.abspath(ann_path) if osp.exists(ann_path) else resolve_path(ann_path)
...
def _resolve_sample_token(
    ...
    if sample_tokens is not None and len(sample_tokens) > 0:
        try:
            ...
            # Debug: silently pass, will try other methods
            pass

    ds = pack.get('dataset', None)

    # 2) Use sample_idx from meta to index into dataset.data_list or data_infos
    ...
    if ds is not None and sample_idx is not None:
        try:
            ...
            if data_list is not None and 0 <= idx < len(data_list):
                info = data_list[idx]
...
def run_manual_benchmark(
    ...
    if len(processed_tokens) > 0:
        ...
        if 'results' in pred_data:
            # Filter results to only include processed tokens
            filtered_results = {
    ...
    if len(processed_tokens) > 0:
        # Monkey-patch the evaluator's __init__ to skip the assertion
        original_init = NuScenesEval.__init__
        filtered_tokens = set(processed_tokens)
        
        def patched_init(self, nusc, config, result_path, eval_set, output_dir, verbose):
            # Call original init but catch the assertion
            try:
                original_init(self, nusc, config, result_path, eval_set, output_dir, verbose)
            except AssertionError as e:
                if "Samples in split doesn't match samples in predictions" in str(e):
                    # Load GT and predictions manually, then filter
                    ...
                    self.pred_boxes, _ = load_prediction(result_path, config.max_boxes_per_sample, DetectionBox, verbose=verbose)
                    
                    # Filter both to only processed tokens
                    self.gt_boxes.boxes = {
                        k: v for k, v in self.gt_boxes.boxes.items()
                        if k in filtered_tokens
                    }
                    self.pred_boxes.boxes = {
                        k: v for k, v in self.pred_boxes.boxes.items()
                        if k in filtered_tokens
                    }
                else:
                    raise
        
        NuScenesEval.__init__ = patched_init
        try:
            ...
            NuScenesEval.__init__ = original_init
    else:
        nusc_eval = NuScenesEval(
            nusc,
            config=cfg_eval,
...
def inference_loop(model: torch.nn.Module,
    ...
    for idx, (token, pts, imgs, meta, paths, _) in pbar:
        ...
        #     )
        if save_images:
            if 'img_norm_cfg' in meta and torch.is_tensor(imgs) and 'lidar2img' in meta:
